...
class AppStatusListenerSuite extends SparkFunSuite with BeforeAndAfter {
  ...
  test("scheduler events") {
    ...
    time += 1
    val stages = Seq(
      new StageInfo(1, 0, 0, "stage1", 4, None, Nil, Nil, "details1"),
      new StageInfo(2, 0, 0, "stage2", 4, None, Nil, Seq(1), "details2"))
    ...
    val newS2 = new StageInfo(oldS2.stageId, 0, oldS2.attemptNumber + 1, oldS2.name, oldS2.numTasks,
      None, oldS2.rddInfos, oldS2.parentIds, oldS2.details, oldS2.taskMetrics)
    ...
    time += 1
    val j2Stages = Seq(
      new StageInfo(3, 0, 0, "stage1", 4, None, Nil, Nil, "details1"),
      new StageInfo(4, 0, 0, "stage2", 4, None, Nil, Seq(3), "details2"))
    ...
    // Stop executors.
    time += 1
    listener.onExecutorRemoved(SparkListenerExecutorRemoved(time, "1", "Test", null))
    listener.onExecutorRemoved(SparkListenerExecutorRemoved(time, "2", "Test", null))
  ...
  test("storage events") {
    ...
    val stage = new StageInfo(1, 0, 0, "stage1", 4, None,
      Seq(rdd1Info, rdd2Info), Nil, "details1")
  ...
  test("eviction of old data") {
    ...
    val stages = Seq(
      new StageInfo(1, 0, 0, "stage1", 4, None, Nil, Nil, "details1"),
      new StageInfo(2, 0, 0, "stage2", 4, None, Nil, Nil, "details2"),
      new StageInfo(3, 0, 0, "stage3", 4, None, Nil, Nil, "details3"))
    ...
    val attempt2 = new StageInfo(3, 0, 1, "stage3", 4, None, Nil, Nil, "details3")
    time += 1
  ...
  test("eviction should respect stage completion time") {
    ...
    val stage1 = new StageInfo(1, 0, 0, "stage1", 4, None, Nil, Nil, "details1")
    val stage2 = new StageInfo(2, 0, 0, "stage2", 4, None, Nil, Nil, "details2")
    val stage3 = new StageInfo(3, 0, 0, "stage3", 4, None, Nil, Nil, "details3")
  ...
  Seq(true, false).foreach { live =>
    test(s"Total tasks in the executor summary should match total stage tasks (live = $live)") {
      ...
      val stage = new StageInfo(1, 0, 0, "stage", 4, None, Nil, Nil, "details")
  ...
  test("storage information on executor lost/down") {
    ...
    // Submit a stage and make sure the RDDs are recorded.
    val rdd1Info = new RDDInfo(rdd1b1.rddId, "rdd1", 2, level, Nil)
    val stage = new StageInfo(1, 0, 0, "stage1", 4, None, Seq(rdd1Info), Nil, "details1")
    listener.onStageSubmitted(SparkListenerStageSubmitted(stage, new Properties()))
  ...
  private def createExecutorRemovedEvent(executorId: Int) = {
    SparkListenerExecutorRemoved(10L, executorId.toString, "test", null)
  }
