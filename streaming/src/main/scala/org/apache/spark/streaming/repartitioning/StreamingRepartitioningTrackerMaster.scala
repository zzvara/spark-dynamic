...
import hu.sztaki.drc.component.{SimpleStream, StreamState, StreamingRepartitioningTrackerMasterHelper}
...
import org.apache.spark.repartitioning.NaiveBatchStrategy._
  ...
  class StreamingListener extends Listener {
      ...
      this.synchronized {
        taskEnd.taskInfo.stageProperties.flatMap(
          ...
          case Some(stream) =>
            ...
            StreamingUtils.updateStreamIDToStreamMap(streamID, stream)
            if (taskEnd.taskType != "ResultTask" && !taskEnd.taskInfo.killed) {
            ...
            if (recordsRead > 0) {
              StreamingUtils.getShuffleHead(StreamingUtils.getDStream(streamID)) match {
                ...
                case None =>
                // in some cases, shuffle read happens, but repartitioning is not required (e.g. dStream.repartition(numParts))
                // do nothing
              }
            }
          case None =>
    ...
    override def onJobEnd(jobEnd: SparkListenerJobEnd): Unit = {
      ...
      _jobData.get(jobEnd.jobId).foreach { masterJobData =>
        ...
        // TODO: repartitionCount is not updated properly?
    ...
    override def onJobStart(jobStart: SparkListenerJobStart): Unit = {
      /**
        ...
        */
      def getDependenciesRecursively(streams: scala.collection.immutable.Set[DStream[_]]):
      scala.collection.immutable.Set[DStream[_]] = {
        if (streams.isEmpty) {
          scala.collection.immutable.Set[DStream[_]]()
        } else {
          streams ++ getDependenciesRecursively(streams.flatMap(_.dependencies))
        }
      }
      ...
      if (configuredRPMode != Mode.Disabled) {
        /**
        ...
        if (streamProperties.isDefined) {
          ...
          val streamID = stream.ID
          val parallelism = stream.numPartitions
          ...
          if (!_streamData.contains(streamID)) {
            ...
            _streamData.update(streamID,
              StreamState[Stream](stream, mutable.Set[Int](jobStart.jobId), parentStreams, scanStrategy))
            ...
            _streamData.update(streamID, {
              _streamData(streamID).addJob(jobStart.jobId).asInstanceOf[StreamState[Stream]]
            })
    ...
    override def onStageSubmitted(stageSubmitted: SparkListenerStageSubmitted): Unit = {
      if (!stageSubmitted.properties.getOrDefault("spark.job.description", "").toString.contains("Streaming")) {
